...
function bracesValid(str) {
  ...
  for(let i = 0; i < str.length ;i++){
    if (str[i] === "("){
      ...
      parenthesisTrack--
      if (parenthesisTrack < 0 || lastOpen[lastOpen.length-1] != "("){
        return false
      }
      else{
        lastOpen.pop()
      ...
      curlyTrack--
      if (curlyTrack < 0 || lastOpen[lastOpen.length-1] != "{"){
        return false
      }
      else{
        lastOpen.pop()
...
// Same thing, but with counters for all 3 types.
// It also needs to check if a bracket starts, a parenthesis starts, and then the bracket end.
// needs a tracker of the last brace type, if a close happens after the wrong type, it errors.